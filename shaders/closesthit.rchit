...
layout(binding = 7, set = 0) readonly buffer MatricesBuffer
{
  InstancesMatrices m[];
}
matrices;
...
Vertex getVertex(ivec3 trianglIndex, uint vertexOffset, vec3 barycentrics)
{
  ...
  vtx.nrm = normalize(vec3(matrices.m[gl_InstanceID].worldIT * vec4(vtx.nrm, 0.0)));
  // Properly transforming the vertex
  vtx.pos = vec3(matrices.m[gl_InstanceID].world * vec4(vtx.pos, 1.0));
...
void main()
{
  ...
  prd.origin         = origin;//v.pos;
  prd.attenuation    = prd.attenuation * diffuse_color;
  prd.countEmitted   = 0;
  ...
  prd.direction = p;
  //compute_default_basis(v.nrm, x, y, z);
  //prd.direction = samplingHemisphere(prd.seed, x, y, z);

  //prd.attenuation = v.nrm;
  //prd.done = 1;
